...
export default function ListadoFraccionamientos({id_usuario,perfil_usuario }: FiltrosConsultaProps,{
    ...
    useEffect(() => {
      ...
      const datos = () => {
        ...
        const fetchResumen = async () => {
            try {
              const response = await fetch(`/api/dashboard/reportes/comisiones/resumen?fInicio=${fInicio}&fFin=${fFin}&idUsuario=${id_usuario}&perfil=${perfil_usuario}`);
              if (!response.ok) {
        ...
        const fetchDetallado = async () => {
            try {
              const response = await fetch(`/api/dashboard/reportes/comisiones/detallado?fInicio=${fInicio}&fFin=${fFin}&usuario=${id_usuario}&perfil=${perfil_usuario}`);
              if (!response.ok) {
                throw new Error(`Failed to fetch data: ${response.status}`);
              }
              const data = await response.json();
    ...
    return (
        <>
            ...
            <Button className="p-6 w-full" onClick={datos} disabled={!date}>VER DATOS</Button>
