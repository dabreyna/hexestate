...
export default function FiltrosConsultaDetalladoDePagos({
  listaFraccionamientos,
}: FiltrosConsultaProps) {
  const seleccionaResultados = useDetalladoDePagosFiltrosConsultaStore(
  ...
  function getDatos() {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `/api/dashboard/reportes/detalladoDePagos?idFraccionamiento=${fraccionamiento}&idManzana=${manzana}&idTerreno=${terreno}&nombreCliente=${nombreCliente}`
        );
        if (!response.ok) {
          throw new Error(`Failed to fetch data: ${response.status}`);
        }
